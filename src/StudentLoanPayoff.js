...
const Input = ({ label, id, ...props }) => (
  <div>
    ...
    <input id={id} {...props} className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
  </div>
);
...
const Select = ({ label, id, children, ...props }) => (
  <div>
    <label htmlFor={id} className="block text-sm font-medium text-slate-700 mb-1">{label}</label>
    <select id={id} {...props} className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
      {children}
    </select>
  </div>
);
...
const ResultsCard = ({ title, plan, warning }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 flex flex-col">
      ...
      {plan.status && <p className="text-xs font-bold text-red-600 my-1">{plan.status}</p>}
      {warning && <p className="text-xs text-amber-700 my-1 flex items-center gap-1"><AlertTriangle size={14}/> {warning}</p>}
      <div className="mt-2 space-y-1 text-sm flex-grow text-slate-600">
        <p><span className="font-semibold text-slate-700">Monthly Payment:</span> {formatCurrency(plan.monthlyPayment)}</p>
        <p><span className="font-semibold text-slate-700">Total Paid:</span> {formatCurrency(plan.totalPaid)}</p>
        <p><span className="font-semibold text-slate-700">{dateLabel}</span> {formatDate(dateToUse)}</p>
        {plan.totalInterest != null && <p><span className="font-semibold text-slate-700">Total Interest:</span> {formatCurrency(plan.totalInterest)}</p>}
      </div>
    </div>
  );
};
...
const PaymentBreakdownVisualizer = ({ baseline, accelerated, totalOriginalBalance, calcMode }) => {
  ...
  return (
    <div className="mt-6 bg-white rounded-lg p-5 border-2 border-blue-100">
      <h4 className="font-bold text-slate-800 mb-4 text-center">Payment Breakdown: Where Your Money Goes</h4>
      
      <div className="flex justify-center gap-6 mb-6 text-sm">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-green-600 rounded"></div>
          <span className="text-slate-600">Principal (Your Debt)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-red-500 rounded"></div>
          <span className="text-slate-600">Interest (Bank Profit)</span>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <div className="flex justify-between items-center mb-2">
            <h5 className="font-semibold text-slate-700 text-sm">Minimum Payments Only</h5>
            <span className="text-xs text-slate-600">Total: ${baselineTotal.toFixed(2)}</span>
          </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs text-slate-600">Total: ${acceleratedTotal.toFixed(2)}</span>
          </div>
...
export default function StudentLoanPayoff() {
  ...
  // Management functions and computed values (unchanged)
  ...
  const privateLoanResults = useMemo(() => {
    ...
    return calculatePrivateLoanPayoff(privateLoans, privateCalcMode, parseFloat(extraPayment || 0), parseInt(privateTargetYear || 0));
  ...
  const refinanceResults = useMemo(() => {
    ...
    return { monthlyPayment, totalPaid, totalInterest: totalPaid - totalFederalBalance, payoffDate };
  ...
  const bestFederalPlan = useMemo(() => {
    ...
    if (refinanceComparisonMode === 'lowestPayment') {
      let lowestPlan = null, lowestPayment = Infinity;
    ...
    if (refinanceComparisonMode === 'lowestTotal') {
      let lowestPlan = null, lowestTotal = Infinity;
      for (const [name, plan] of Object.entries(plans)) {
  ...
  const allFederalPlans = useMemo(() => Object.keys(eligiblePlans.plans), [eligiblePlans.plans]);
  const federalAcceleratedResults = useMemo(() => {
    ...
    const weightedAvgRate = federalLoans.reduce((acc, loan) => acc + (parseFloat(loan.balance || 0) * (parseFloat(loan.rate || 0) / 100)),0) / totalFederalBalance;
    if (federalCalcMode === 'extra') {
      if (!federalExtraPayment || parseFloat(federalExtraPayment) <= 0) return null;
      return calculateAcceleratedPayoff(totalFederalBalance, weightedAvgRate, selectedPlan, parseFloat(federalExtraPayment), selectedPlan.isIdr || false);
    }
    if (federalCalcMode === 'target') {
      ...
      return calculateTargetYearPayment(totalFederalBalance, weightedAvgRate, basePayment, parseInt(federalTargetYear));
    }
    return null;
  ...
  const handleDownloadSchedule = (principal, annualRate, monthlyPayment, fileName) => {
    ...
    const csvContent = [headers, ...schedule.map(row => `${row.paymentNumber},${row.paymentDate},${row.paymentAmount.toFixed(2)},${row.principal.toFixed(2)},${row.interest.toFixed(2)},${row.remainingBalance.toFixed(2)}`)].join("\n");
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold text-slate-800">The Payoff Climb</h1>
          <p className="mt-2 text-lg text-slate-600">Your journey to student loan freedom</p>
        </div>

        {/* Financial Profile */}
        <Card>
          <h2 className="text-2xl font-semibold text-slate-800 mb-4">Your Financial Profile</h2>
        ...
        <Card>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold text-slate-800">Your Loans</h2>
            <button onClick={addLoan} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition shadow-sm">
              <PlusCircle size={20} /> Add Loan
            </button>
          </div>
          <div className="space-y-6">
            {loans.length === 0 && <p className="text-slate-500 text-center py-4">Add a loan to get started.</p>}
            {loans.map((loan, index) => (
              <div key={loan.id} className="bg-slate-100 p-4 rounded-lg border border-slate-200">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-semibold text-slate-700">Loan #{index + 1}</h3>
                  <button onClick={() => removeLoan(loan.id)} className="text-red-500 hover:text-red-700"><XCircle size={20} /></button>
                </div>
                {!loan.type ? (
                  <div className="flex gap-4">
                    <button onClick={() => updateLoan(loan.id, 'type', 'Federal')} className="flex-1 py-2 px-4 border border-slate-300 rounded-md hover:bg-slate-200 text-slate-700">Federal Loan</button>
                    <button onClick={() => updateLoan(loan.id, 'type', 'Private')} className="flex-1 py-2 px-4 border border-slate-300 rounded-md hover:bg-slate-200 text-slate-700">Private Loan</button>
                  </div>
                ) : loan.type === 'Federal' ? (
        ...
        {showContaminationQuestion && (
          <Card className="bg-amber-50 border border-amber-200">
            <h3 className="font-semibold text-amber-800 mb-2">Future Federal Loans</h3>
            <p className="text-sm text-amber-700 mb-4">This question is required because you have loans from before July 1, 2026.</p>
        ...
        {federalLoans.length > 0 && Object.keys(eligiblePlans.plans).length > 0 && (
          <Card>
            <div className="mb-6">
              <h2 className="text-2xl font-semibold text-slate-800">Federal Loan Repayment Plans</h2>
              <p className="text-sm text-slate-600">Based on a total federal balance of ${totalFederalBalance.toLocaleString()}</p>
            </div>
        ...
        {federalLoans.length > 0 && allFederalPlans.length > 0 && (
          <Card>
            <h2 className="text-2xl font-semibold text-slate-800 mb-2">Federal Loan Acceleration Strategy</h2>
            <p className="text-sm text-slate-600 mb-6">See how extra payments or a target payoff year can help you become debt-free faster.</p>
            
            {!showFederalStrategy ? (
              <div className="text-center mt-4">
                <button onClick={() => setShowFederalStrategy(true)} className="flex items-center justify-center gap-2 w-full md:w-auto px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition">
              ...
              <div className="bg-green-50 p-6 rounded-xl border border-green-200">
                <div className="mb-6 bg-white p-4 rounded-lg border border-slate-200">
                  <p className="text-sm font-semibold text-slate-700 mb-3">Calculate based on:</p>
                ...
                {federalAcceleratedResults?.error && (
                  <div className={`p-4 rounded-lg mb-6 ${federalAcceleratedResults.alreadyMeetsTarget ? 'bg-amber-100 border border-amber-200 text-amber-800' : 'bg-red-100 border border-red-200 text-red-800'}`}>
                  ...
                  <div className="opacity-0 animate-fade-in-up" style={{ animationFillMode: 'forwards' }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="bg-white rounded-lg p-4 border-2 border-green-500">
                        <h3 className="font-bold text-lg text-green-700 mb-3">{federalCalcMode === 'extra' ? 'With Extra Payment' : 'To Meet Target'}</h3>
                        <div className="space-y-2 text-sm">
                    ...
                    <div className="bg-white rounded-lg p-4 border border-slate-200 mb-4">
                      <h4 className="font-bold text-slate-800 mb-2">{federalCalcMode === 'extra' ? 'Savings Summary' : 'Target Achievement'}</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        {federalCalcMode === 'target' && federalAcceleratedResults.requiredExtraPayment && (<p><span className="font-semibold">Required Extra Payment:</span> <span className="text-green-700 font-bold">${federalAcceleratedResults.requiredExtraPayment.toFixed(2)}/month</span></p>)}
                        <p><span className="font-semibold">Interest Saved:</span> <span className="text-green-700 font-bold">${(federalAcceleratedResults.baseline.totalInterest - federalAcceleratedResults.accelerated.totalInterest).toFixed(2)}</span></p>
                        <p><span className="font-semibold">Time Saved:</span> <span className="text-green-700 font-bold">{Math.floor(Math.abs(federalAcceleratedResults.savings.monthsSaved) / 12)} years, {Math.abs(federalAcceleratedResults.savings.monthsSaved) % 12} months</span></p>
                      </div>
                    </div>
                    <DebtFreeTimeline baselineDate={federalAcceleratedResults.baseline.payoffDate} acceleratedDate={federalAcceleratedResults.accelerated.payoffDate} />
                    <div className="mt-6 text-center">
                      <button onClick={() => handleDownloadSchedule(totalFederalBalance, (federalLoans.reduce((acc, loan) => acc + (parseFloat(loan.balance) * parseFloat(loan.rate) / 100),0) / totalFederalBalance), federalAcceleratedResults.accelerated.monthlyPayment, 'federal-loan-schedule.csv')} className="flex items-center justify-center gap-2 w-full md:w-auto px-4 py-2 text-sm bg-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-300 transition">
                        <Download size={16} /> Download Full Schedule
                      </button>
                    </div>
                    {federalAcceleratedResults.paidOffBeforeForgiveness && (
                      <div className="bg-blue-100 rounded-lg p-4 border border-blue-200 mt-4 text-sm text-blue-800">
        ...
        {privateLoans.length > 0 && (
          <Card>
            <h2 className="text-2xl font-semibold text-slate-800 mb-2">Private Loan Payoff Strategy</h2>
            <p className="text-sm text-slate-600 mb-2">Based on a total private balance of ${totalPrivateBalance.toLocaleString()}</p>
            <p className="text-sm text-slate-500 mb-6">Strategy: Debt Avalanche (paying highest interest rate first)</p>
            {!showPrivateStrategy ? (
              <div className="text-center mt-4">
                <button onClick={() => setShowPrivateStrategy(true)} className="flex items-center justify-center gap-2 w-full md:w-auto px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition">
                  Analyze Payoff Strategies <ChevronsRight size={20} />
                </button>
              </div>
            ) : (
              <div className="bg-green-50 p-6 rounded-xl border border-green-200">
                ...
                {privateLoanResults?.error && (
                  <div className={`mt-4 p-4 rounded-lg ${privateLoanResults.alreadyMeetsTarget ? 'bg-amber-100 border border-amber-200 text-amber-800' : 'bg-red-100 border border-red-200 text-red-800'}`}>
                ...
                {privateLoanResults && !privateLoanResults.error && (
                  <div className="mt-6 opacity-0 animate-fade-in-up" style={{ animationFillMode: 'forwards' }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="bg-white rounded-lg p-4 border-2 border-green-500">
                        <h3 className="font-bold text-lg text-green-700 mb-3">{privateCalcMode === 'extra' ? 'With Extra Payment' : 'To Meet Target'}</h3>
                        <div className="space-y-2 text-sm">
                    ...
                    {(parseFloat(extraPayment || 0) > 0 || privateCalcMode === 'target') && (
                      <div className="bg-white rounded-lg p-4 border border-slate-200">
                        <h4 className="font-bold text-slate-800 mb-2">{privateCalcMode === 'extra' ? 'Savings Summary' : 'Target Achievement'}</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          {privateCalcMode === 'target' && privateLoanResults.requiredExtraPayment && (<p><span className="font-semibold">Required Extra Payment:</span> <span className="text-green-700 font-bold">${privateLoanResults.requiredExtraPayment.toFixed(2)}/month</span></p>)}
                          ...
                          <p><span className="font-semibold">Time Saved:</span> <span className="text-green-700 font-bold">{Math.floor(privateLoanResults.savings.monthsSaved / 12)} years, {privateLoanResults.savings.monthsSaved % 12} months</span></p>
                        </div>
                      </div>
                    )}
                    {privateLoanResults && !privateLoanResults.error && (parseFloat(extraPayment || 0) > 0 || privateCalcMode === 'target') && (
                      <>
                        ...
                        <DebtFreeTimeline baselineDate={privateLoanResults.baseline.payoffDate} acceleratedDate={privateLoanResults.accelerated.payoffDate} />
                         <div className="mt-6 text-center">
                          <button onClick={() => handleDownloadSchedule(totalPrivateBalance, (privateLoans.reduce((acc, loan) => acc + (loan.balance * loan.annualRate), 0) / totalPrivateBalance), privateLoanResults.accelerated.monthlyPayment, 'private-loan-schedule.csv')} className="flex items-center justify-center gap-2 w-full md:w-auto px-4 py-2 text-sm bg-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-300 transition">
                            <Download size={16} /> Download Full Schedule
                          </button>
                        </div>
                      </>
        ...
        {federalLoans.length > 0 && (
          <Card>
            <h2 className="text-2xl font-semibold text-slate-800 mb-6">Strategic Tools</h2>
            <div className="bg-red-50 border border-red-200 p-6 rounded-lg">
              <h3 className="text-xl font-semibold text-red-800 mb-4">Refinancing Simulator</h3>
              <div className="mb-6 bg-white p-4 rounded-lg border border-slate-200">
                <p className="text-sm font-semibold text-slate-700 mb-3">Compare refinancing based on:</p>
                <div className="space-y-2">
                  ...
                  {refinanceComparisonMode === 'custom' && (
                    <div className="ml-6 mt-2">
                      <select value={customPlanSelection} onChange={(e) => setCustomPlanSelection(e.target.value)} className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
              ...
              {refinanceResults && bestFederalPlan && (
                <div className="mb-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-blue-100 border-2 border-blue-500 rounded-lg p-5">
                    ...
                    <div className="bg-red-100 border-2 border-red-500 rounded-lg p-5">
                      <div className="flex items-center gap-2 mb-3"><span className="text-2xl">⚠️</span><h4 className="font-bold text-red-800">Refinanced Option</h4></div>
