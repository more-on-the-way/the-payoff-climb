                  ...
                  {selectedScenarios.map(scenario => (
                    <div key={scenario.id} className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-xl border-2 border-indigo-200">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-white p-4 rounded-lg">
                          <div className="text-sm text-gray-600 mb-1">Interest Rate Range</div>
                          <div className="text-lg font-bold text-rose-600">
                            {Math.min(...scenario.savedLoans.map(l => parseFloat(l.loanRate))).toFixed(2)}% - {Math.max(...scenario.savedLoans.map(l => parseFloat(l.loanRate))).toFixed(2)}%
                          </div>
                        </div>
                        <div className="bg-white p-4 rounded-lg">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            (${scenario.resultsData.totalMinPayment.toFixed(2)} min + ${scenario.resultsData.extra.toFixed(2)} extra)
                          </div>
                        </div>
                        <div className="bg-white p-4 rounded-lg">
                        ...
                        <div className="bg-white p-4 rounded-lg">
                          <div className="text-sm text-gray-600 mb-1">Snowball Payoff</div>
                          <div className="text-xl font-bold text-blue-600">{scenario.resultsData.snowballMonths} months</div>
                          <div className="text-sm text-gray-700 mt-1">
                            Interest: ${scenario.resultsData.snowballTotalInterest.toFixed(2)}
                          </div>
                        </div>
                        <div className="bg-white p-4 rounded-lg">
                          <div className="text-sm text-gray-600 mb-1">Total Cost (Avalanche)</div>
                          <div className="text-xl font-bold text-purple-600">
                            ${(scenario.resultsData.totalBalance + scenario.resultsData.avalancheTotalInterest).toFixed(2)}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div key={loan.id} className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <div className="font-semibold">${(parseFloat(loan.balance) + calculateGraceInterest(loan)).toFixed(2)}</div>
                    </div>
                    <div>
                ...
                <div className="bg-gray-100 p-6 rounded-xl">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Current Plan Total Cost:</span>
                      <span className="font-semibold">${(results.totalMinPayment * results.avalanche.months + results.avalanche.totalInterest).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Refinanced Total Cost:</span>
                      <span className="font-semibold">${refinanceResults.totalPaid.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            {idrResults && (
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-purple-500 to-pink-600 text-white p-6 rounded-xl">
                  <h3 className="text-lg font-semibold mb-4">New Plan (Est. 2026)</h3>
                  <div className="text-4xl font-bold mb-2">${idrResults.newPlan.toFixed(2)}/mo</div>
                  <p className="text-sm opacity-90">5% of discretionary income (estimated)</p>
                </div>
                ...
                {results && (
                  <div className="bg-gray-100 p-6 rounded-xl">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span>Lowest IDR Option (New Plan):</span>
                        <span className="font-semibold">${idrResults.newPlan.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Monthly Difference:</span>
                        <span className={`font-semibold ${
                          (results.totalMinPayment + results.extra) > idrResults.newPlan ? 'text-green-600' : 'text-red-600'
                        }`}>
                          ${Math.abs((results.totalMinPayment + results.extra) - idrResults.newPlan).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <p className="text-sm text-gray-700">
                    <strong>Note:</strong> These are estimates. IDR plans may offer loan forgiveness after 20-25 years. 
                    Consult with your loan servicer for accurate calculations and plan details.
                  </p>
                </div>
              </div>
            )}
