...
function simulateRAP({ principal, annualRate, initialAgi, familySize, stateOfResidence, filingStatus }) {
    ...
    for (let month = 1; month <= forgivenessMonths; month++) {
        ...
        // Interest Subsidy is implicit: unpaid interest is not added to the balance.
...
export const calculatePlans = (financialProfile, loans) => {
  ...
  let graduatedCurrentPayment = standardMonthly * 0.5;
  for (let month = 1; month <= 120; month++) {
      if(graduatedBalance <= 0) break;
      const interest = graduatedBalance * (weightedAverageRate / 12);
      const principalToPay = graduatedCurrentPayment - interest;
      const actualPrincipalPaid = Math.min(graduatedBalance, principalToPay > 0 ? principalToPay : 0);
      graduatedBalance -= actualPrincipalPaid;
      graduatedTotalPaid += graduatedCurrentPayment;
      if (month % 24 === 0 && month < 120) graduatedCurrentPayment = Math.min(graduatedCurrentPayment * 1.2, standardMonthly * 3); // More realistic increase
  }
  plans['Graduated'] = { monthlyPayment: `Starts at ~$${(standardMonthly * 0.5).toFixed(2)}, increases every 2 years`, totalPaid: graduatedTotalPaid, payoffDate: new Date(new Date().setFullYear(new Date().getFullYear() + 10)) };
  ...
  const icrMonthly = Math.min(
      ...
      calculateAmortizedPayment(totalFederalBalance, weightedAverageRate * 1.2, 12) // Income factor approximation
  );
  plans['ICR'] = {
